...
{
	unicast_net* pmt_net = (unicast_net*)ptr;
	Frame frame;

	while (1)
	{
		unsigned long address = 0;
		int frame_size = pmt_net->reciv((char*)&frame, &address, sizeof(Frame));   // waiting for frame 
...
{
	SERV_IP = strtok(arguments, " ");
	int id = atoi(strtok(NULL, " "));

	fprintf(f, "SERV_IP = %s\n", SERV_IP);
	fprintf(f, "USTAWIONE ID = %d\n", id);
...
void VirtualWorldCycle()
{
	...
	frame.iID = my_vehicle->iID;   
	frame.type = FrameType::NORMAL;// my object identifier
